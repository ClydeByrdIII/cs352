...
public class RegisterAllocator {
    ...
    class CFNode {
        ...
        Map<Integer, HashSet<Variable>> defs, ins, outs, uses;
        Map<Integer, HashSet<InfoNode>> preds, succs;
        Variable master;

        public CFNode(Variable var) {
            master = var;
            preds = new HashMap<Integer, HashSet<InfoNode>>();
            succs = new HashMap<Integer, HashSet<InfoNode>>();
    ...
    public void initVariables() {
        ...
        for(SSAStatement s : block) {
            var = new Variable(s);
            variables.add(var);
            infonodes.add(new InfoNode(s, var));
        }
    }
    ...
    public void buildInterference() {
        for(TempNode temp : tempnodes) {
          //  if(temp.getInfoNode().getCFNode().ins.get(temp.getInfoNode().getSSA().))
           //interference.add(temp.);
        }
    }
    ...
    public void simplify(int freeRegisters) {
        ...
        /*while(!graph.isEmpty()) {
            TempNode min = findMinDegreeNode();
            degree = min.getDegree();
         
            if(degree >= k) {
                min = graph.leastUsedNode();
                stack.addLast(min);
            } else {
                stack.push(min);
            }
    ...
    public void performSpills(Set<TempNode> actualSpills) {
        ...
        SSAStatement left = temp.getSSA(spill);
        // check arrayList
